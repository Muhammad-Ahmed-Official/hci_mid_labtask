...
import { ROOM_COLORS, calculatePolygonArea, distance } from '@/lib/floorPlanUtils';
import { FloorPlanState } from '@/lib/useFloorPlan';

export default function RightPanel({ state }: { state: FloorPlanState }) {
  ...
  const polyIndex = selectedPoly as number;
  ...
  return (
    <div className="w-56 border-l border-border bg-card p-4 select-none hidden lg:flex flex-col gap-4 overflow-y-auto">
      ...
      <div className="space-y-1.5">
        ...
        <Input
          value={poly.label || ''}
          onChange={(e) => updatePolylineProps(polyIndex, { label: e.target.value })}
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={(e) => updatePolylineProps(polyIndex, { color: e.target.value })}
        ...
        <div className="space-y-1.5">
          ...
          <div className="grid grid-cols-4 gap-1.5">
            {ROOM_COLORS.map((rc, i) => (
              <button
                key={i}
                onClick={() => updatePolylineProps(polyIndex, {
                  fillColor: rc.fill,
                  roomColorIndex: i,
                })}
      ...
      <div className="mt-auto">
        <Button
          ...
          onClick={() => deletePolyline(polyIndex)}
        >
